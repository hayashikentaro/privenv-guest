...
const forbiddenPatterns: Array<{ pattern: RegExp; reason: string }> = [
  ...
  { pattern: /(^|\/)privenv\.host\.json$/, reason: "Host config must not be packed" },
  { pattern: /(^|\/)\.privenv\/vault\.json$/, reason: "Host vault must not be packed" },
  { pattern: /(^|\/)\.privenv\/audit\.log\.jsonl$/, reason: "Host audit log must not be packed" },
